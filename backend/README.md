...
### 2. Workout & Meal Scoring (Content-Based Filtering)
...
### 3. Repetition Counter Integration
In the video feedback pipeline, the rep-counting logic has been refactored to use the **`RepetitionCounter`** module. Instead of manually inspecting phase state changes in a dictionary inside the loop, the system streams joint angles/coordinates to `RepetitionCounter.add_frame()` and queries `get_rep_count()` to fetch the updated count dynamically.
...
| :---------- | :--------------------------- | :-------------------------------------------------------------------- | :------------------------- | :-------------------------------------------- |
| **POST**    | `/users/`                    | Registers a new session & profile. Computes target calories.          | `UserCreateRequest` (body) | `{"session_id": str, "target_calories": int}` |
...
| **PUT**     | `/users/{session_id}/goal`   | Updates user's goal type (e.g., fat loss) and updates calorie budget. | `UpdateGoalRequest` (body) | `UserResponse`                                |
...
| HTTP Method | Route                              | Description                                                                | Query Parameters                                                                                         | Request Payload / Response Model                                                |
| :---------- | :--------------------------------- | :------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------ |
| **GET**     | `/workouts/`                       | Paginated catalog queries.                                                 | `workout_type` (str), `body_part` (str), `equipment` (str), `level` (str), `limit` (int), `offset` (int) | Returns matching exercises: `{"total": int, "workouts": List[WorkoutResponse]}` |
...
| **GET**     | `/workouts/{workout_id}`           | Fetch a single workout details by database id.                             | None (pass `workout_id` in path)                                                                         | `WorkoutResponse`                                                               |
| **POST**    | `/workouts/recommend/{session_id}` | Predicts top-K workouts filtered & scored for this user.                   | `top_k` (int, default 5, range 1-50)                                                                     | Request Body: None<br>Response: `List[WorkoutResponse]`                         |
| **GET**     | `/workouts/filter/{session_id}`    | Hard-filtered exercises matching user's gear & level (unscored).           | None                                                                                                     | `List[WorkoutResponse]`                                                         |
| **POST**    | `/workouts/plan/{session_id}`      | Generates a structured multi-day training routine based on split settings. | `top_k` (int, default 5, range 1-50)                                                                     | Request Body: None<br>Response: `WorkoutPlanResponse`                           |
...
| HTTP Method | Route                           | Description                                                                 | Query Parameters                                                                   | Request Payload / Response Model                                                                       |
| :---------- | :------------------------------ | :-------------------------------------------------------------------------- | :--------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------- |
| **POST**    | `/meals/fetch/{session_id}`     | Fetches recipes matching user preferences with hard constraints (unscored). | `num_fetch` (int, default 20, range 5-100)                                         | Request Body: None<br>Response: `{"fetched": int, "after_filter": int, "meals": List[RecipeResponse]}` |
| **POST**    | `/meals/recommend/{session_id}` | Generates top-K custom-tailored, ranked meals based on preferences.         | `top_k` (int, default 5, range 1-50)<br>`num_fetch` (int, default 20, range 5-100) | Request Body: None<br>Response: `List[RecipeResponse]`                                                 |
...
| HTTP Method | Route                                  | Description                                                                  | Request Payload               | Response Model                            |
| :---------- | :------------------------------------- | :--------------------------------------------------------------------------- | :---------------------------- | :---------------------------------------- |
| **POST**    | `/meal-plan/generate/{session_id}`     | Builds a daily menu (Breakfast, Lunch, Dinner, Snacks) with target matching. | `MealRecommendRequest` (body) | `MealPlanResponse`                        |
...
| HTTP Method | Route                            | Description                                                             | Request Payload                 | Response Model                                       |
| :---------- | :------------------------------- | :---------------------------------------------------------------------- | :------------------------------ | :--------------------------------------------------- |
| **POST**    | `/feedback/meal/{session_id}`    | Logs a like/dislike for a meal recipe.                                  | `MealFeedbackRequest` (body)    | `{"status": "ok", "recipe_id": str, "action": str}`  |
| **POST**    | `/feedback/workout/{session_id}` | Logs a like/dislike for a workout.                                      | `WorkoutFeedbackRequest` (body) | `{"status": "ok", "workout_id": str, "action": str}` |
