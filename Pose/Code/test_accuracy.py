...
def test_all_accuracy():
    ...
    test_data_dir = os.path.abspath(os.path.join(base_dir, '../Data'))
    ...
    valid_exercises = [ex for ex in sorted(exercises) if ex.lower() in EXERCISE_TO_CONFIG]
    ...
    for ex_idx, exercise_name in enumerate(valid_exercises, 1):
        ...
        videos = (
            glob.glob(os.path.join(ex_dir, "*.mp4")) +
            glob.glob(os.path.join(ex_dir, "*.avi")) +
            glob.glob(os.path.join(ex_dir, "*.mov")) +
            glob.glob(os.path.join(ex_dir, "*.MOV"))
        )
        ...
        for vid_idx, video_path in enumerate(sorted(videos), 1):
            ...
            frames, bad_frames, frame_details, _ = get_inference(
                video_path, exercise_name, headless=True, frame_skip=frame_skip
            )

            ex_total += frames
            ex_bad += bad_frames

            # Only write rows for processed frames (at the correct stride)
            for frame_idx in range(frame_skip + 1, frames + 1, frame_skip + 1):
                is_good = frame_idx not in frame_details
                csv_rows.append({
                    "exercise": exercise_name,
                    "videoName": video_name,
                    "frame": frame_idx,
                    "good/bad": "good" if is_good else "bad",
